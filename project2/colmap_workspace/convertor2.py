...
# -*- coding: utf-8 -*-
"""
Convert COLMAP (images.txt + cameras.txt) into SuperGlue --eval input file.
...
def main():
    parser = argparse.ArgumentParser(description="Generate SuperGlue --eval input from COLMAP outputs")
    ...
    parser.add_argument("--output", default="demo_full.txt", help="Output txt file path")
    ...
    if args.mode == "sequential":
        for i in range(len(image_names) - 1):
            pairs.append((image_names[i], image_names[i + 1]))
    elif args.mode == "full":
    ...
    with open(args.output, "w") as f:
        for name0, name1 in pairs:
            ...
            T0, T1 = im0['T'], im1['T']
            # Relative pose: T_0to1 = T1 * inv(T0)
            # issue: AUC@5==0 反
    ...
    print(f"✅ Generated {len(pairs)} pairs with intrinsics and poses -> {args.output}")
